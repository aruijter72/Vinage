...
const App = {
  ...
  renderView() {
    ...
    switch (this.view) {
      ...
      case 'settings-plan':          el.innerHTML = this.buildSettingsSubView('plan'); break;
      case 'settings-preferences':   el.innerHTML = this.buildSettingsSubView('preferences'); break;
      case 'settings-ai':            el.innerHTML = this.buildSettingsSubView('ai'); break;
      case 'settings-sync':          el.innerHTML = this.buildSettingsSubView('sync'); break;
      case 'settings-notifications': el.innerHTML = this.buildSettingsSubView('notifications'); break;
      case 'settings-data':          el.innerHTML = this.buildSettingsSubView('data'); break;
      case 'settings-about':         el.innerHTML = this.buildSettingsSubView('about'); break;
  ...
  _delegateClick(e) {
    ...
    switch (action) {
      ...
      case 'enter-demo':          this.enterDemo(); break;
      case 'reset-demo':          this.resetDemo(); break;
      case 'exit-demo':           this.exitDemo(); break;
      ...
      case 'go-settings-plan':          this.navigate('settings-plan'); break;
      case 'go-settings-preferences':   this.navigate('settings-preferences'); break;
      case 'go-settings-ai':            this.navigate('settings-ai'); break;
      case 'go-settings-sync':          this.navigate('settings-sync'); break;
      case 'go-settings-notifications': this.navigate('settings-notifications'); break;
      case 'go-settings-data':          this.navigate('settings-data'); break;
      case 'go-settings-about':         this.navigate('settings-about'); break;
      case 'show-upgrade':              this.navigate('upgrade'); break;
      case 'back-from-upgrade':         this.navigate('settings-plan'); break;
...
Wine: ${[name, producer, vintage, region, country, grapes].filter(Boolean).join(', ')}`;
  ...
  buildCellarDetail() {
    ...
    <div class="rack-container">
      ...
      <div class="rack-zoom-container${is3D ? ' rack3d-wrap' : ''}" id="rack-zoom-container">
  ...
  _build3DRackHtml(c) {
    ...
    <div class="rack-3d-container">
      <iframe id="rack3d-iframe" class="rack3d-frame"
        src="./wijnrek_3d.html?v=${Date.now()}">
      </iframe>
  ...
  _onRack3DLoad(cellarId) {
    ...
    iframe.contentWindow.postMessage({ type: 'vinage-slots', slots, cellarId, nx: cols, nz: rows, rackType: c.type || 'grid' }, '*');
  },
  ...
  async findPairings() {
        ...
        <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px">
            ...
            ${tier === 'perfect'
              ...
              : `<span class="pairing-acceptable-badge">${this.t('pairing.acceptable')}</span>`}
    ...
    if (perfect.length === 0 && acceptable.length === 0) {
      html += `<div class="pairing-no-match">
        <div style="font-size:1.4rem;margin-bottom:6px">🍷</div>
        <div style="font-weight:600;margin-bottom:4px">${this.t('pairing.noMatch')}</div>
        <div style="font-size:.85rem;color:var(--text-lt)">${this.t('pairing.noMatchSub')}</div>
      </div>`;
    } else {
      html += `<div class="pairing-section-title">${this.t('pairing.fromCellar')}</div>`;
      html += perfect.map(_wineCard).join('');
      html += acceptable.map(_wineCard).join('');
    }
  ...
  buildSettingsSubView(section) {
    const nl = this.lang === 'nl';
    const title = {
      plan:          nl ? 'Abonnement'      : 'Subscription',
      preferences:   nl ? 'Taal & weergave' : 'Language & display',
      ai:            nl ? 'AI & sleutels'   : 'AI & API keys',
      sync:          nl ? 'Sync & delen'    : 'Sync & sharing',
      notifications: nl ? 'Meldingen'       : 'Notifications',
      data:          nl ? 'Gegevens'        : 'Data',
      about:         nl ? 'Over & juridisch': 'About & legal',
    }[section] || 'Settings';

    const header = `
      <div class="settings-sub-header">
        <button class="settings-sub-back" data-action="back-to-settings">‹ ${this.t('settings.title')}</button>
        <span class="settings-sub-title">${title}</span>
      </div>`;

    switch (section) {
      case 'plan':          return header + this._buildPlanSubPage();
      case 'preferences':   return header + this._buildPreferencesSubPage();
      case 'ai':            return header + this._buildAiSubPage();
      case 'sync':          return header + this._buildSyncSubPage();
      case 'notifications': return header + this._buildNotifSubPage();
      case 'data':          return header + this._buildDataSubPage();
      case 'about':         return header + this._buildAboutSubPage();
  ...
  _buildPlanSubPage() {
    ...
    <div class="settings-section settings-plan-card">
      ...
      <div class="settings-plan-actions">
        ...
        ${plan.id !== 'free' ? `<button class="btn-reset-plan" data-action="reset-plan">${this.t('plan.resetPlan')}</button>` : ''}
      </div>` : ''}
    </div>`;
  },
  ...
  _buildAiSubPage() {
    ...
    const aiContent = signedIn ? `
      <div class="settings-ai-proxy-notice">
        <span style="font-size:1.1rem">✦</span>
        <div>
          <div style="font-weight:600;font-size:.88rem">${nl ? 'AI werkt automatisch' : 'AI works automatically'}</div>
          <div style="font-size:.78rem;color:var(--text-lt);margin-top:2px">${nl
      ...
      <details style="margin-top:10px">
        <summary style="font-size:.78rem;color:var(--text-lt);cursor:pointer;list-style:none;padding:4px 0">
          ${nl ? '▸ Eigen API-sleutel gebruiken (optioneel)' : '▸ Use your own API key (optional)'}
        </summary>
    ...
    <div class="settings-section">
      ...
      <div class="origo-status">
        ...
        <div>
          <div class="origo-status-title">${nl ? 'Actief' : 'Active'}</div>
          <div class="origo-status-sub">${nl
  ...
  _buildSyncSubPage() {
    // Reuse _buildSyncSection but strip the outer settings-section wrapper
    // by calling it directly — the sub-page header already provides context
    return this._buildSyncSection();
  },

  _buildNotifSubPage() {
    return this._buildNotifSection() || `
    <div class="settings-section">
      <p style="color:var(--text-lt);font-size:.88rem;text-align:center;padding:24px 0">
        ${this.lang === 'nl'
  ...
  _buildDataSubPage() {
    ...
    <div class="settings-section">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ...
        <button class="btn btn-ghost btn-full" data-action="export-csv">📊 ${nl ? 'Exporteer als CSV' : 'Export as CSV'}</button>
  ...
  _buildAboutSubPage() {
    ...
    <div class="settings-section">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ...
        <button class="btn btn-ghost btn-full" data-action="preview-consent" style="font-size:.82rem;color:var(--text-lt)">
          👁 ${nl ? 'Toestemmingsscherm bekijken' : 'Preview consent screen'}
        </button>
      </div>
    </div>
    <div class="about-info">
      ...
      <button class="btn btn-ghost btn-full" data-action="show-help"
        ...
        📖 ${nl ? 'Hulp & Functies' : 'Help & Features'}
      </button>
      <div style="font-size:.78rem;color:var(--text-lt);margin-top:10px;text-align:center;letter-spacing:.04em"
        ...
        ${this.t('settings.version')} · ${nl ? 'JOUW WIJN. JOUW COLLECTIE.' : 'YOUR WINE. YOUR COLLECTION.'}
      </div>
      <div id="beta-code-row" style="display:none;margin-top:12px;gap:8px;align-items:center">
        <input id="beta-code-input" class="form-control" type="text"
               placeholder="${nl ? 'Toegangscode…' : 'Access code…'}"
        ...
        <div class="about-content" style="text-align:left">
          ...
          <div class="help-section">
            ...
            <p style="font-size:.86rem;color:var(--text-md);line-height:1.6;margin-bottom:12px">
              ${nl
                ? `Vinage slaat je collectie standaard lokaal op je apparaat op. Voor het <strong>delen van de kelder</strong> is een centrale, online database nodig — anders ziet geen enkel apparaat de wijzigingen van een ander. Die database is <strong>al ingebouwd in de app</strong>: de beheerder van Vinage heeft dit vooraf opgezet via Firebase (een clouddienst van Google). Jij als gebruiker hoeft daar niets voor in te stellen. Het enige dat je nodig hebt is een account (Google of e-mail) — daarmee weet het systeem bij welk huishouden je hoort en welke gegevens jij mag zien.`
                : `Vinage stores your collection locally on your device by default. To <strong>share a cellar</strong>, a central online database is needed — without one, no device can see another's changes. That database is <strong>already built into the app</strong>: the person who manages Vinage set it up in advance using Firebase (a cloud service by Google). You as a user don't need to configure anything. All you need is an account (Google or email) — that's how the system knows which household you belong to and which data you're allowed to see.`}
            </p>
            <ul class="help-list">
              <li><strong>${nl?'Stap 1 — Aanmelden':'Step 1 — Sign in'}</strong><br>
                ${nl?'Ga naar <em>Instellingen → Sync &amp; Delen</em> en tik op <em>Inloggen met Google</em> of <em>Inloggen met e-mail</em>. Doe dit op alle apparaten die je wilt koppelen (bijv. jij en je partner).':'Go to <em>Settings → Sync &amp; Sharing</em> and tap <em>Sign in with Google</em> or <em>Sign in with email</em>. Do this on every device you want to connect (e.g. you and your partner).'}
              </li>
